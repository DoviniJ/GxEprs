...
#' This function outputs the p value of permuted model in the target dataset, using pre-generated GWAS and/or GWEIS summary statistics, and Polygenic Risk Scores (PRSs) of all the individuals. Note that the input files used in this function can be generated by using GWAS_quantitative and/or GWEIS_quantitative, and PRS_quantitative functions. It is recommended to run this function, if you choose to fit 'PRS_gxe x E' interaction component (i.e. novel proposed model, Model 5) when generating risk scores. If the 'PRS_gxe x E' term is significant in Model 5, and insignificant in Model 5* (permuted p value), consider that the 'PRS_gxe x E' interaction component is actually insignificant (always give priority to the p value obtained from the permuted model).
...
#' add_score = "B_add.sscore", gxe_score = "B_gxe.sscore")
#' x
#' }
